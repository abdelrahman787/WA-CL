...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions | jq '.[].status'
...
curl -X POST -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/restart
...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/qr

# Display QR in terminal (requires qrencode)
curl -s -H "X-API-Key: $API_KEY" \
  "http://localhost:3000/api/sessions/{sessionId}/qr?format=raw" | qrencode -t ANSI
```
...
# Session connected
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId} | jq '.status'
# Expected: "CONNECTED"
...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/metrics/memory
...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions?sort=memory
...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/webhooks

# 2. Check recent webhook logs
curl -H "X-API-Key: $API_KEY" \
  "http://localhost:3000/api/sessions/{sessionId}/webhooks/{webhookId}/logs?status=failed&limit=20"
...
curl -X POST -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/webhooks/{webhookId}/test

# 5. Fix based on cause:

# A. Update webhook URL
curl -X PATCH http://localhost:3000/api/sessions/{sessionId}/webhooks/{webhookId} \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
...
curl -X POST -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/webhooks/{webhookId}/disable

curl -X POST -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/webhooks/{webhookId}/enable

# 6. Retry failed deliveries
curl -X POST -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/webhooks/{webhookId}/retry-failed
```
...
curl -H "X-API-Key: $API_KEY" \
  "http://localhost:3000/api/sessions/{sessionId}/webhooks/{webhookId}/logs?limit=5" | jq '.[].status'
...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions | jq '[.[] | select(.status == "CONNECTED")] | length'
...
# 10. Verify version
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/health/detailed | jq '.version'
...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/default/messages?limit=1
```
...
[← 17 - Plugin Architecture](./17-plugin-architecture.md) · [📚 Documentation Index](./README.md) · [19 - Glossary →](./19-glossary.md)
