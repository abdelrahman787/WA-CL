...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/health
...
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/messages/{messageId}

# Check queue status
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/queues/status

# Check rate limit status
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/rate-limit
```
...
# Check webhook logs
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{sessionId}/webhooks/{webhookId}/logs?limit=20
...
# Get group list
curl -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions/{id}/groups
...
> 2. Set URL: `http://openwa:3000/api/sessions/{id}/messages`
...
[← 12 - Migration Guide](./12-migration-guide.md) · [📚 Documentation Index](./README.md) · [14 - API Collection →](./14-api-collection.md)
