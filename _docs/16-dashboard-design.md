...
export function useWebSocket(apiKey: string) {
  ...
  const connect = useCallback(() => {
    const wsUrl = `${import.meta.env.VITE_WS_URL || 'ws://localhost:3000'}/ws?apiKey=${apiKey}`;
...
[← 15 - SDK Design](./15-sdk-design.md) · [📚 Documentation Index](./README.md) · [17 - Plugin Architecture →](./17-plugin-architecture.md)
