...
## 12.3 Database Migration: SQLite → PostgreSQL
...
## 12.4 Session Auth State Transfer
  ...
  for (const sessionId of sessionIds) {
    try {
      ...
      const exportResponse = await axios.get(
        `${config.sourceUrl}/api/sessions/${sessionId}/export`,
        { headers: { 'X-API-Key': config.sourceApiKey } }
      );

      // 3. Import to target
      await axios.post(
        `${config.targetUrl}/api/sessions/import`,
        exportResponse.data,
...
for sessionId in $(curl -s -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions | jq -r '.data[].id'); do
...
curl -s -H "X-API-Key: $API_KEY" \
  http://localhost:3000/api/sessions | jq '.[] | {id, status}'
...
# 6. Verify
sleep 10
curl -f http://localhost:3000/health && echo "✅ Rollback successful"
```
...
## 12.9 Troubleshooting Migration Issues
...
[← 11 - Testing Strategy](./11-testing-strategy.md) · [📚 Documentation Index](./README.md) · [13 - Troubleshooting FAQ →](./13-troubleshooting-faq.md)
